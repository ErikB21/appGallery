@if($category->category_name)
    <h1 class="text-center mb-5">Modify Category</h1>
...
@endif
    ...
    <div class="form-group mt-3 d-flex justify-content-center">
        @if($category->category_name)
            <button class="btn btn-warning mt-2 mx-3"><i class="bi bi-pen"></i> Update</button>
    </form>
            <form  action="{{ route('categories.destroy', $category->id) }}" method="POST" class="row">
                @csrf
                @method('DELETE')
                <button class="mx-3 btn btn-danger mt-2" id="{{$category->id}}"><i class="bi bi-trash"></i> Delete Category</button>
            </form>
        @else
            <button class="btn btn-primary mt-2"><i class="bi bi-save"></i> Save</button>
        @endif
    </div>